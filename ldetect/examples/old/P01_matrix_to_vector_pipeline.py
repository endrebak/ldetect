...
def multiple_pipelines(dataset_path, name, begin_list, end_list, img='no', orient='diag', red='sum', delta=1000000):
    ...
    for begin, end in zip(begin_list, end_list):
        ...
        pipeline(dataset_path, name, begin-int(delta), begin+int(delta), img, orient, red, begin, comment=str(run)+'-begin')
        pipeline(dataset_path, name, end-int(delta), end+int(delta), img, orient, red, end, comment=str(run)+'-end') 
